...
import { MONOLITHBOND_TOKEN } from './utils/Constants'
import { loadOrCreateToken } from './utils/Tokens'
import { createDailyBondRecord } from './utils/DailyBond'
import { getMonolithUSD } from './utils/Price'
...
export function handleDeposit(call: DepositCall): void {
  ...
  let token = loadOrCreateToken(MONOLITHBOND_TOKEN)
  ...
  deposit.value = getMonolithUSD(call.inputs._amount)
...
export function handleRedeem(call: RedeemCall): void {
  ...
  redemption.token = loadOrCreateToken(MONOLITHBOND_TOKEN).id;
